  ...
  return (
    <article className="cd-basis-full sm:cd-basis-1/3 md:cd-basis-1/3 lg:cd-basis-1/3 cd-p-[0.5rem]">
      <div className="cd-p-[1rem] cd-text-center cd-justify-center cd-items-center cd-flex cd-flex-col cd-gap-y-[1rem] cd-shadow-lg cd-rounded-md cd-border cd-border-gray-200">
        ...
        <button
          className="cd-transition-colors cd-duration-200 cd-w-full cd-bg-red-500 hover:cd-bg-red-700 cd-text-white cd-px-[0.5rem] cd-py-[0.25rem] cd-rounded-md cd-mt-2 cd-text-sm"
          onClick={() => {
            if (confirm('¿Estás seguro de eliminar este perfil?') && id) {
              DataRepo.deleteUser(id)
                .then(() => alert('Perfil eliminado'))
                .catch(() => alert('Error eliminando perfil'));
            }
          }}
        >
          Eliminar perfil
